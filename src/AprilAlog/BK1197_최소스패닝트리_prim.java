...
public class BK1197_최소스패닝트리_prim {
    ...
    private static int prim() {
        ...
        while (!pq.isEmpty()){
            ...
            System.out.println("지금 확인하는 정점은  "+ edge.to);

            // 이미 방문한 정점인 경우
            if(visited[edge.to]){
                System.out.println(edge.to + "는 이미 방문했어요!!!!");
                continue;
            }

            ret += edge.value;
            System.out.println("ret은  "+ ret);
            ...
            for (int i = 0 ; i < adj[edge.to].size(); i++){
                ...
                if (visited[next.to]){
                    System.out.println(next.to + "는 이미 방문했어요!!!!");
                    continue;
                }
                pq.add(next);
                System.out.println("pq를 보여줘     "+ pq.peek().to);
            }
        }
        return ret;
    }
